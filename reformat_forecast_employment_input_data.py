...
def pre_process_lms_sic():
    ...
    for region in region_corr['RGN21NM']:
        df = pd.read_csv(LMS_INPUT_DIR / fr'LMS_SIC_Ind2\LMS_Ind2_{region}.csv',
    ...
    sic_rgns = pd.merge(sic_rgns, lms_sic_corr, left_on='Industry',
                        right_on='Labour Market & Skills', how='left').dropna()
    ...
    for year in f_years.keys():
        ...
        f_year_control = (f_year_control[['LU_SIC_1_digit', 'region', year]].
                          rename(columns={'LU_SIC_1_digit': 'sic_1_digit'}))
        ...
        if count > 0:
            LOGGER.error(f'Extrapolating the LM&S growth to {year} results in negative values')
...
def pre_process_lms_soc():
    ...
    # Read in and format the LM&S data for each region
    for region in region_corr['RGN21NM']:
        df = pd.read_csv(LMS_INPUT_DIR / fr'LMS_SOC\LMS_Occ_T1_{region}.csv',
                         header=[0], skiprows=[1, 11, 12])
        df['region'] = region
        soc.append(df)
    ...
    soc_rgns = pd.merge(soc_rgns, lms_soc_corr, left_on='Levels (000s)',
                        right_on='Occupation', how='left').dropna()
    # TODO question, 2025 to 2035 or 2015 to 2035?
    soc_rgns = soc_rgns[['SOC', 'region', '2025', '2035']].groupby(
        by=['SOC', 'region'], as_index=False)[['2025', '2035']].sum()
    ...
    # Calculate % growth for 1 year
    soc_rgns['%_growth_1yr'] = ((soc_rgns['2035'] - soc_rgns['2025']) / soc_rgns['2025']) / 10
    ...
    for year in f_years.keys():
        ...
        f_year_control = f_year_control.astype({'SOC': 'int'}).rename(columns={'SOC': 'soc'})
        # Remap region back to codes
        f_year_control['region'] = f_year_control['region'].map(
            dict((x, y) for y, x in dict(sorted(region_corr.values.tolist())).items()))
