...
def _load_population_data(census_microdata_path, QS401_path, QS606_path, QS609_path, NTEM_population_path):
    ...
    :param census_microdata_path: Path of census microdata .csv
    :param QS401_path: Path of QS401UK (usual residents by accommodation type) .csv
    :param QS606_path: Path of QS606UK (usual residents by occupation type) .csv
    :param QS609_path: Path of QS609UK (usual residents by HRP's NSSeC) .csv
    :param NTEM_population_path: Path of NTEM derived population .csv

    :return:
        Formatted dataframes of: census_microdata, QS401, QS606, QS609, NTEM_population
    """
...
def _segment_and_scale_ntem_population(NTEM_population, f_tns_daghe, ntem_tt_lookup, geography_lookup):
    ...
    return NTEM_pop_scaled, NTEM_pop_actual
...
def _segment_qs(census_QS, NTEM_population, segment_letter, column_to_segment_mapping, segment_to_id_mapping):
    """
    Resegment QS population and scale to match NTEM population.

    :param census_QS:
        Formatted QS dataframe, as generated by _load_population_data.
        Keys: ['MSOA', 'Census_Population'] + segmentation population columns
    :param NTEM_population:
        Formatted NTEM population.
        Keys: ['z', 'd', 'r', 'MSOA', 'C_NTEM']
    :param segment_letter:
        Letter to represent the segmentation.
        e.g. 'a' for age, 'g' for gender
    :param column_to_segment_mapping:
        Dictionary of segmented population column names to group together.
        e.g. {'New_Terrace': 'Terrace', 'Old_Terrace': 'Terrace', 'New_Flat': 'Flat', 'Old_Flat': 'Flat'}
    :param segment_to_id_mapping:
        Grouped population segment names to their enumeration.
        e.g. {'Terrace': 1, 'Flat': 2}

    :return:
        Dataframe with population by QS segmentation, scaled to NTEM total population.
        Keys: ['z', 'd', 'r', segment_letter, 'Persons']
    """
...
def create_ipfn_inputs(census_and_by_lu_obj):
    ...
    scaled_NTEM_population, actual_NTEM_population = _segment_and_scale_ntem_population(
